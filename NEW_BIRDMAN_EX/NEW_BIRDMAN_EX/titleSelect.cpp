...
bool TITLE_SELECT::Update()
{
	...
	imeshX = (imeshX >= -_DEF_SCREEN_X / 2) ? imeshX -= _BGMESH_SPEED : _DEF_SCREEN_X / 2;//背景網の移動
...
void TITLE_SELECT::Render()
{
	...
	sscDrawGraph(imeshX, _DEF_SCREEN_Y / 2, 1.0, 0.0,iImage[_TITLE_SELECT_BGNET][0], TRUE, FALSE );//背景網1
	sscDrawGraph(imeshX + _DEF_SCREEN_X, _DEF_SCREEN_Y / 2, 1.0, 0.0,iImage[_TITLE_SELECT_BGNET][0], TRUE, FALSE );//背景網2
